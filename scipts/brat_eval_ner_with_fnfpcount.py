...
"""
The script was originally developed for 2018 N2C2 challenge

We modified this script for evaluation NER and relation extraction performance for any brat formatted data
...
fn_dict=dict()
fp_dict=dict()
def evaluate_result(corpora, annotations, mode='strict', verbose=False):
    gs_tags, gs_rels = annotations
    tag_type=None
    for g, s in corpora.docs:
        evaluator = SingleEvaluator(g, s, 2, mode, tag_type, verbose=verbose)
#         print(evaluator.scores['tags']['tp'])
#         print(g.basename)
        if g.basename not in fn_dict.keys():
            fn_dict.update({g.basename:evaluator.scores['tags']['fn']})
            fp_dict.update({g.basename:evaluator.scores['tags']['fp']})
        else:
            fn_dict[g.basename]=fn_dict[g.basename]+evaluator.scores['tags']['fn']
            fp_dict[g.basename]=fp_dict[g.basename]+evaluator.scores['tags']['fp']
    fn_dict_sorted=dict(sorted(fn_dict.items(), key=lambda item: item[1],reverse=True))
    fp_dict_sorted=dict(sorted(fp_dict.items(), key=lambda item: item[1],reverse=True))
    print('****** false negetive count ******')
    print(fn_dict_sorted)
    print('****** false positive count ******')
    print(fp_dict_sorted)
...
#     print(evaluator_tag_l.scores['tags']['tp'])
...
def eval_files(f1, f2, verbose=False):
    ...
    if corpora.docs:
        ...
        evaluate_result(corpora, annotations, verbose=verbose)
