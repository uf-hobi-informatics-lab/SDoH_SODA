...
class SentenceBoundaryDetection:
    ...
    # def set_special_rules(self, regex):
    #     self.special = True
    #     self.new_rules.add(regex)
        ...
        for i, line in enumerate(text): # E. coli
            ...
            for j, word in enumerate(words):
                if self.special:
                    ...
                    # DAILYATENOLOL => DAILY ATENOLOL
                    if word.lower().startswith("daily") and not word.lower().endswith("daily") and not word.lower().endswith("."):
                        s, e = re.search("daily", word, re.IGNORECASE).span()
                        word_list.extend([word[:e], " ", word[e:], " "])
                        continue

                    if word.lower().startswith("qhs") and not word.lower().endswith("qhs") and not word.lower().endswith("."):
                        s, e = re.search("qhs", word, re.IGNORECASE).span()
                        word_list.extend([word[:e], " ", word[e:], " "])
                        continue

                    if word.lower().startswith("bid") and not word.lower().endswith("bid") and not word.lower().endswith("."):
                        s, e = re.search("bid", word, re.IGNORECASE).span()
                        word_list.extend([word[:e], " ", word[e:], " "])
                        continue

                    if word == "SUPPERASCORBIC":
                        word_list.extend(["SUPPER", " ", "ASCORBIC", " "])
                        continue

                    if re.match("[A-Za-z]+[0-9]+", word):
                        # xxx000 => xxx 000
                        s, e = re.search("[A-Za-z]+", word).span()
                        word_list.extend([word[:e], " ", word[e:], " "])
                        continue

                    if word == "insulinSS":
                        word_list.extend(["insulin", " ", "SS", " "])
                        continue

                    if word == "3LNS":
                        word_list.extend(["3L", " ", "NS", " "])
                        continue

                    if word == "ccNS.":
                        word_list.extend(["cc", " ", "NS", " ", ".", " "])
                        continue

                    if word == "CAP.SR":
                        word_list.extend(["CAP.SR", " "])
                        continue

                    if word == "CAP.SRFolic":
                        word_list.extend(["CAP.SR", " ", "Folic", " "])
                        continue
                        ...
                        if self.__num_dot(word) == 1:
                            ...
                            if word[-1] == ".":
                                ...
                                if re.match("^[0-9]+[.][0-9]+x[0-9]+[.][0-9]+$", word):
                                    ...
                                    # TODO: a.b  a...b .....a two cases
                                    word_seg = re.split("(\.+)",word)
                                    tlt = []
...
on [**12-29**], """
    ...
    text3 = """take after meals #90 CAP.SRFolic Acid  TARTRATE25
    3LNS acetaminophen500  lipitor20 dailyATENOLOL atenolol50 insulinSS ccNS."""
